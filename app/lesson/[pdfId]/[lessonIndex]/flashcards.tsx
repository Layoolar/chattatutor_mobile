...
import { markFlashcardsCompleted } from "@/lib/lesson-progress";
...
function RateButton({ label, variant, disabled, onPress }: RateButtonProps) {
  ...
  return (
    <Animated.View style={[{ flex: 1 }, animatedStyle]}>
      <Pressable
        ...
        disabled={disabled}
        className={`h-12 items-center justify-center rounded-full ${borderless ? "" : "border"} ${
          disabled
            ? `${borderless ? "" : "border-slate-200"} bg-slate-100 opacity-60`
            : enabledClass
        }`}
      >
        <Text className={`text-sm font-semibold ${disabled ? "text-slate-500" : enabledTextClass}`}>
          {label}
        </Text>
      </Pressable>
    </Animated.View>
...
function FlipCard({ front, back, isFlipped, onTap, entryKey }: FlipCardProps) {
  ...
  const entryStyle = useAnimatedStyle(() => ({
    opacity: entry.value,
    transform: [
      { scale: interpolate(entry.value, [0, 1], [0.92, 1]) },
      { translateY: interpolate(entry.value, [0, 1], [16, 0]) },
    ],
  }));

  return (
    <Animated.View style={[{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0 }, entryStyle]}>
...
export default function LessonFlashcardsScreen() {
  ...
  const sessionCards = useMemo(
    () =>
      reviewRound
        ...
        : flashcards,
    [reviewRound, reviewQueue, flashcards],
  );
  const currentCard =
    currentIndex >= 0 && currentIndex < sessionCards.length
      ? sessionCards[currentIndex]
      : null;
  const sessionComplete = !loading && flashcards.length > 0 && currentCard === null;
  const progressTarget =
    sessionCards.length > 0 ? Math.min(currentIndex / sessionCards.length, 1) : 0;
  const nextLabel = reviewRound ? "Review round" : "Main deck";

  useEffect(() => {
    progress.value = withTiming(progressTarget, { duration: 360, easing: Easing.out(Easing.cubic) });
  ...
  useEffect(() => {
    ...
    sfx.complete();
    if (pdfId && !Number.isNaN(index)) {
      markFlashcardsCompleted(String(pdfId), index);
    }
  ...
  }, [sessionComplete, completionPulse]);

  const progressBarStyle = useAnimatedStyle(() => ({
    width: `${progress.value * 100}%`,
  }));

  const completionIconStyle = useAnimatedStyle(() => ({
    transform: [{ scale: completionPulse.value === 0 ? 0.6 : completionPulse.value }],
    opacity: completionPulse.value === 0 ? 0 : Math.min(1, completionPulse.value),
  }));
  ...
  const advanceCard = useCallback(
    (rating: FlashcardRating) => {
      ...
      recordFlashcardInteraction(String(pdfId), index, currentCard.id, rating).catch(() => {
        if (!syncErrorShown) {
  ...
  const completedCount = sessionComplete ? sessionCards.length : Math.min(currentIndex + 1, sessionCards.length);
          ...
          <View className="flex-1 gap-5">
            ...
            >
              ...
              <View className="mt-6 flex-row gap-2">
                ...
                <StatPill
                  label="Card"
                  value={`${completedCount}/${sessionCards.length}`}
                />
              </View>
            ...
            {sessionComplete ? (
              <View className="relative flex-1">
                ...
                <View className="rounded-3xl border border-slate-200 bg-white p-6 gap-4">
                  <Animated.View style={completionIconStyle}>
              ...
              <>
                ...
                <View className="gap-3">
                  ...
                  <View className="flex-row gap-2">
                    <RateButton label="Hard" variant="hard" disabled={!isFlipped} onPress={() => handleRate("hard")} />
                    <RateButton label="Good" variant="good" disabled={!isFlipped} onPress={() => handleRate("good")} />
                    <RateButton label="Easy" variant="easy" disabled={!isFlipped} onPress={() => handleRate("easy")} />
                  </View>
                </View>
              </>
            ) : null}
